 ...
 */
class CustomsupportpagesController extends SiteBaseController {
	/**
	...
	{
        if( isset($_GET['alias']) && ( $model = CustomSupportPages::model()->find('alias=:alias AND language=:lang', array( ':lang' => Yii::app()->language,  ':alias' => $_GET['alias'] )) ) )
		{	
			// Page is active?
			if( !$model->status )
			{
