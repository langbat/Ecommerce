...
class MemberShopController extends AdminBaseController {
    public function init()
	{
		parent::init();
		
		$this->breadcrumbs[ Yii::t('global', 'Member Shops') ] = array('memberShop/index');
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['MemberShop']))
		{
            ...
            if(!empty($uploadedFileB)) {
                ...
                $modeloldB       = $this->loadModel($id);
                $model->banner   = $modeloldB->banner ;
            }
			if($model->save()){
			    if(!empty($uploadedFile)) {
                    $uploadedFile->saveAs(Yii::app()->basePath.'/../uploads/logoshop/'.$model->image);
                } 
                if(!empty($uploadedFileB)) {
                    $uploadedFileB->saveAs(Yii::app()->basePath.'/../uploads/logoshop/'.$model->banner);
                } 
				$this->redirect(array('view','id'=>$model->id));
            }
		}
	...
	public function actionDelete($id)
	{
		if(Yii::app()->request->isPostRequest)
		{
			...
			if($this->loadModel($id)->delete()){
			 ProductsShop::model()->deleteAll('shop_id=?',array($id));
			}
