...
class ProductsController extends AdminBaseController {
    public function init()
	{
		...
		$this->breadcrumbs[ Yii::t('global', 'Products') ] = array('products/index');
		$this->pageTitle[] = Yii::t('global', 'Products');
	}
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Products']))
		{
            ...
            //$model->video = $_POST['Products']['video'];
            if($_POST['options_video'] == 'link'){
                $model->video = $_POST['Products']['video'];
            } else if($_POST['options_video'] == 'choose'){
                $model->video = $_POST['Products']['video'];
            } else if($_POST['options_video'] == 'upload'){
                $uploadedVideo=CUploadedFile::getInstance($model,'video2');
                if(!empty($uploadedVideo)) {
                    $rnd = rand(0,9999);
                    $fileVideo = "{$rnd}-{$uploadedVideo}";  // random number + file name
                    $model->video = $fileVideo;
                }
                else{
                    $fileVideo = '';
                }
			...
			if($model->save()){
                if($_POST['Products']['tag'] != ''){
                    $tags = explode(',',$_POST['Products']['tag']);
                    Tags::model()->saveTags($tags,$model->id);
                }
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['Products']))
		{
            if($_POST['Products']['tag'] != ''){
                $tags = explode(',',$_POST['Products']['tag']);
                Tags::model()->updateTags($tags,$model->id);
            }
	...
	{
	   $schedule_shows = ScheduleShows::model()->findAllByAttributes(array('product_id'=>$id));
        ...
        elseif(Yii::app()->request->isPostRequest)
		{
           ...
           if($schedule_shows){
            ScheduleShows::model()->deleteAll('product_id = ?' , array($id));
           }
            //ProductComments::model()->deleteAll('product_id = ? AND type = ?' , array($id, '1'));
	...
	public function actionIndex()
	{
        ...
        $model->is_active=Products::STATUS_ACTIVE;
